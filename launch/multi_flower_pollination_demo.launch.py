...
def generate_launch_description():
    ...
    real_sense_node = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(get_package_share_directory('vision_pkg'), 
                         'launch', 'isaac_ros_realsense_on_link1.launch.py')
        )
    )
    ...
    rs_sahi_global_flower_pose_server_node = Node(
        package='proj_farmhand',
        executable='rs_sahi_global_flower_pose_server',
        name='rs_sahi_global_flower_pose_server'
    )
