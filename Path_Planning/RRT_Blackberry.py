...
import cv2
import numpy as np
from YOLO_UR5.yolov5.detect import yolo_detection
import torch
import torch.backends.cudnn as cudnn
from YOLO_UR5.models.common import DetectMultiBackend
from YOLO_UR5.utils.dataloaders import IMG_FORMATS, VID_FORMATS, LoadImages, LoadStreams
from YOLO_UR5.utils.general import (LOGGER, check_file, check_img_size, check_imshow, check_requirements, colorstr, cv2,
                            increment_path, non_max_suppression, print_args, scale_coords, strip_optimizer, xyxy2xywh)
from YOLO_UR5.utils.plots import Annotator, colors, save_one_box
from YOLO_UR5.utils.torch_utils import select_device, time_sync
...
def yolo_detection(source, weights):
    ...
    global MODEL
    if MODEL is None:
        MODEL = DetectMultiBackend(weights, device=device, dnn=False, data="./YOLO_UR5/yolov5/data/data.yaml", fp16=False)
    model = MODEL
...
def get_berry_boxes_and_centroids(imageName, cam): 
    ...
    while final_boxes is None:
        result, img = cam.read()
        cv2.imwrite("./Run_Images/Raw_Berry_Image.jpg", img)
        t3 = time.perf_counter()
        boxes = yolo_detection("./Run_Images/Raw_Berry_Image.jpg", YOLO_WEIGHT_FILE)
        t4 = time.perf_counter()
        yoloDuration = t4 - t3

        # Saving an annotated image to see which berries were detected
        if boxes is not None:
            frame = cv2.imread("./Run_Images/Raw_Berry_Image.jpg")
            for line in boxes:
                cv2.rectangle(frame, (int(line[0]), int(line[1])), (int(line[2]), int(line[3])), (0,255,0), 3)
                cv2.imwrite(imageName, frame) # Save annotation as a new image
            final_boxes = boxes
        
        else:
            print("Did not detect berries, resetting camera and trying again.")
            cam.release()
...
for i in range(centroids1.shape[0]):
    ...
    goal_pos = adjustments[0][0] # Set goal position

    print("\nGoal Position of Blackberry: ", goal_pos)

    rrt, path, path_points = rrt_class.main(adjustments[0][0]) # Generate list of points for final path and 
    ...
    rrt_class.main2(rrt, path, goal_pos)
