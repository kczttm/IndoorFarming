...
class RRT:
    ...
    def steer(self, from_node, to_node, extend_length=float(0.004)):
    ...
    def plot_final_path(self, path, start, goal, workspace_bounds, obstacle_list):
        ...
        if path is not None:
            ...
            ax.scatter(start[0], start[1], start[2], c='g', marker='o', label='Start Position') # Plot starting point
            ax.scatter(goal[0], goal[1], goal[2], c='r', marker='o', label='Blackberry Location') # Plot goal point
            ...
            ax.plot_surface(x_cylinder, y_cylinder_, z_cylinder, color='k', alpha=0.5, label='Cylindrical Obstacle') # Plot cylinder
            ...
            start_handle = plt.Line2D([0], [0], marker='o', color='g', label='Start Position')
            goal_handle = plt.Line2D([0], [0], marker='o', color='r', label='Blackberry Location')
            cylinder_handle = plt.Line2D([0], [0], color='k', linestyle='-', label='Cylindrical Obstacle')
            handles = [start_handle, goal_handle, cylinder_handle]
            labels = [handle.get_label() for handle in handles]
            ax.legend(handles=handles, labels=labels, loc='upper right', bbox_to_anchor=(1.1, 1.0)) # Create legend for plot
            ...
            x_cylinder, y_cylinder, z_cylinder = self.cylinder_along_z(obstacle_list[0].center[0], obstacle_list[0].center[1], obstacle_list[0].radius, obstacle_list[0].height) # Create plottable x, y, z values for cylinder
            ax.plot_surface(x_cylinder, y_cylinder, z_cylinder, color='k', alpha=1, label='Cylindrical Obstacle') # Plot cylinder
...
def main(goal_pos):
    ...
    start_pos = [0, 0, 0]
    workspace_bounds = np.array([[-0.4, 0.4], [-0.4, 0.4], [-0.4, 0.4]])
    rrt = RRT(start=start_pos, goal=goal_pos, rand_area=[-2, 15], obstacle_list=obstacle_list, workspace_bounds=workspace_bounds, robot_radius=0.035)
    ...
    if path is None:
        ...
        return rrt, path, pose_list # path is list of 1 x 3 arrays representing the points in space that compose the final path, pose_list is a list of 1 x 6 points to include roll, pitch, yaw for UR5

def main2(rrt, path, goal_pos):

    center = np.array([-0.01, 0, 0.08])
    radius = 0.01
    height = 0.5
    direction = np.array([0, 1, 0])
...
if __name__ == '__main__':
    ...
    rrt, path, pose_list = main(goal_pos)
