...
class RRT_1:
    ...
    def __init__(self,
        ...
        """
        self.start = self.Node_1(start[0], start[1])
        self.end = self.Node_1(goal[0], goal[1])
        self.min_rand = rand_area[0]
        self.max_rand = rand_area[1]
        if play_area is not None:
            self.play_area = self.AreaBounds_1(play_area)
        else:
            self.play_area = None
    ...
    def steer(self, from_node, to_node, extend_length=float("inf")):
        ...
        new_node = self.Node_1(from_node.x, from_node.y)
    ...
    def generate_final_course(self, goal_ind):
        ...
        print(path) # The list is in order from goal position to starting position
    ...
    def get_random_node(self):
        if random.randint(0, 100) > self.goal_sample_rate:
            rand_node = self.Node_1(
                random.uniform(self.min_rand, self.max_rand),
                random.uniform(self.min_rand, self.max_rand))
        else:  # goal point sampling
            rand_node = self.Node_1(self.end.x, self.end.y)
        return rand_node

    def draw_graph(self, rand_node=None):
        plt.clf()
        ...
        if rand_node is not None:
            plt.plot(rand_node.x, rand_node.y, "^k")
            if self.robot_radius > 0.0:
                self.plot_circle(rand_node.x, rand_node.y, self.robot_radius, '-r')
