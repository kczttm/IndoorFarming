...
class YoloVisualServoActionServer(Node):
    ...
    def __init__(self, base):
        ...
        # note that twist is naturally in tool frame
        self.command.reference_frame = Base_pb2.CARTESIAN_REFERENCE_FRAME_TOOL
    ...
    def IBVS_pos_only(self, box_cx, box_cy, box_length, des_x, des_y, des_length):  
        ...
        xy_gain = 1.0
        z_gain = 1.0
        pos_diff_endo = np.array([(des_x-box_cx)/focal_length*desired_z*xy_gain,
                            (des_y-box_cy)/focal_length*desired_z*xy_gain,
                            (des_length-box_length)/2/focal_length*desired_z*z_gain])

        # reducing the pos error (in EE frame!!)
        pos_diff = np.dot(self.EE_endo_mtx[:3,:3], pos_diff_endo)
    ...
    def get_largest_flower_box_center_and_length(self, boxes):
        ...
        if flower_box is not None:
            ...
            return center_x, center_y, length
        else:
    ...
    def image_callback(self, msg):
        ...
        center_x, center_y, length = self.get_largest_flower_box_center(boxes)
        if center_x is not None:
            self.no_flower_count = 0
            # get desired center from frame
            des_x, des_y = frame.shape[1] / 2, frame.shape[0] / 2
            des_length = 0.75*frame.shape[0] # % of the frame height'
