...
from proj_farmhand.flower_center_tracking import get_flower_center, calc_xy_plane_focus_error, calc_xy_plane_pose_error
...
global H_wd_ee_initial  # created for the new endo --- flower line
...
def robot_micro_adjust(SerialObj, REAL_FLOWER=False):
    ...
    time.sleep(0.5)
    if REAL_FLOWER:
        motor_command(SerialObj, 'Z', 0) # better to have blurry image than focused
    else:
    ...
    with DeviceConnection.createTcpConnection(tcp_args) as router:
        ...
        while True:
            ...
            #######TODO: Implement the focus controller here
...
def robot_pollinate_single_flower(rs_flower_loc=None, _lambda = 0.5, serial_obj=None, euler_x=0, zoom_in=False, REAL_FLOWER=False):
    ...
    if rs_flower_loc is not None:
        ...
        # des_z[2] = 0
        # comment out the above line to keep the z-axis of the endoscope pointing at the flower
        
        ## Instead of drawing a line between the realsense and the flower,
        ## below code draws a line between the endocope and the flower
        H_wd_ee = H_wd_ee_initial
        H_wd_endo = H_wd_ee @ tf_to_hom_mtx(EE_endo_tf)
        des_z = rs_flower_loc - H_wd_endo[:3,3]
        ...
        # euler_y = np.radians(30)  # hardcode the y tilt to 30 degrees
    ...
    if REAL_FLOWER:
        ...
        fork_lower = 0.000 # the raise of the fork from the lowest point of the flower (0.001 mm) y is pointing down
    else:
        fork_depth = -0.002  # the depth of the fork in the flower (0.007 mm) z is pointing out
...
def main():
    ...
    H_wd_ee_initial = get_current_EE_pose()[0]
    ...
    if not skip_rs:
        ...
        flower_poses_wd, joint_angles_init, H_wd_rs = realsense_get_flower_poses(sahi_n_slices = rs_nslice)
        ...
        if flower_poses_wd is None:
            ...
            np.save(os.path.join(exp_data_dir, "H_wd_ee_initial.npy"), H_wd_ee_initial)
    else:   
        run_id = int(np.load(os.path.join(exp_data_dir, "run_id.npy")))
        run_id -= 1
        np.save(os.path.join(exp_data_dir, "run_id.npy"), run_id)
        ...
        H_wd_ee_initial = np.load(os.path.join(exp_data_dir, "H_wd_ee_initial.npy"))
    ...
    for i in range(flower_index, len(flower_poses_wd)):
        ...
        # tilt_angle = opt_ee_y_tngle*180/np.pi)
