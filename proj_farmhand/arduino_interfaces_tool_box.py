...
def auto_focus(SerialObj, cam_id=4, predefined_pos=0):
    ...
    data_counter = 0
    focus_score = 0
    focus_score_max = 50 # real flower threshold: 100 | fake flower threshold: 50 # for 200 by 200 crop
    fc_temp = 0
    fc_sum = 0
    focus_timer = 0
    zoom_val = 0
    ...
    while True:
        ...
        if data_counter >= 10:
            ...
            if not hold_zoom and not focus_wait:
                ...
                if zoom_e < 0:
                    zoom_e = 0
                kp = 1 / 20 # max zoom val (180 deg) / max variance val (in the thousands)
                e_kp = zoom_e * kp # real flower k: 1/40 | fake flower k: 1/20
                # print(e_kp)
