...
from gen3_7dof.tool_box import get_endoscope_tf_from_yaml, tf_to_hom_mtx, get_polli_fork_tf_from_yaml
...
class YoloContactServoActionServer(Node):
    # Will make into action server later
    def __init__(self, base):
        super().__init__('yolo_contact_servo_action_server')
        ...
        self.EE_polli_fork_mtx = tf_to_hom_mtx(EE_polli_fork_tf)

        # control constants
        self.max_vel = 0.1  # m/s  0.5 is max
        #max_vel = 0.1
        self.max_w = 50.0  # ~ 50 deg/s
        self.kp_pos = 2.5
        self.kp_ang = 4.0

        # contact detection parameters
        self.flower_box = None
    ...
    def get_largest_flower_box(self, boxes):
        # input: boxes (object detection results) (in device)
        # output: [top_left_x, top_left_y, bot_right_x, bot_right_y] (as numpy)
        max_area = 0
        flower_box = None
        ...
        if flower_box is not None:
            ...
            return np.round(flower_box).astype(int)
        else:
            return None

    
    def calculate_focus_score(self, image, blur=9):
    ...
    def image_callback(self, msg):
        ...
        box = self.get_largest_flower_box(boxes)
        if box is not None:
            self.flower_box = box
            self.no_flower_count = 0
        ...
        if self.flower_box is not None:
            # crop the image to the flower box
            # print(self.flower_box)
            roi = frame[self.flower_box[1]:self.flower_box[3], self.flower_box[0]:self.flower_box[2]]
            focus_score = self.calculate_focus_score(roi)
            self.get_logger().info('Focus Score: %f' % focus_score)
...
def main(args=None):
    ...
    with DeviceConnection.createTcpConnection(tcp_args) as router:
        ...
        robot_controller = YoloContactServoActionServer(base)
